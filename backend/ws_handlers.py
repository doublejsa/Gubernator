...
async def chat_ws_handler(ws: WebSocket, user: User, db: AsyncSession, sessions_ref: dict,
    ...
    agent_block = (
        ...
        f"- To change which LLM the agent itself runs on: {agent_info['model_hint']} "
        f"Discover the exact syntax ONCE with --help, then [REMEMBER] it so it is never "
        f"rediscovered. This is a routine config change — it must not take more than a few steps.\n"
